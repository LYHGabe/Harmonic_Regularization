...
class ICNN_optim:
    ...
    def optim_Euc_reg(self,q_in_reg,qtraj ,q_dot,dt,q_in_boundary,q_dot_boundary,penalty,penalty_boundary, learning_rate = 1e-2,epoch=10000,
        ...
        #                         shuffle=True,
        #                         num_workers=8)
        qmin_long = np.zeros([2,batch_size])
        qmax_long = np.zeros([2,batch_size])
        for i in range(batch_size):
            qmin_long[:,i] = self.qmin
            qmax_long[:,i] = self.qmax
        ...
        for i in range(epoch):
            current_grid_data = torch.tensor(((np.array(self.qmax)-np.array(self.qmin))*np.random.rand(batch_size,2)+np.array(self.qmin)),dtype=dtype,device=self.device)
            current_grid_data.requires_grad=True
            #current_grid_data = torch.tensor(((np.array(self.qmax)-np.array(self.qmin))*np.random.rand,batch_size,2)+np.array(self.qmin),dtype=dtype,device = self.device)
            qdot_bound_pred = self.model.f_forward(q_in_boundary,self.Xstable).to(self.device)
            temp0_boundary = torch.sqrt((qdot_bound_pred**2)[:,0]+(qdot_bound_pred**2)[:,1])
            ...
            loss_reg = torch.sum(qdot_grad_norm)/batch_size#*self.dA/2*(self.nq1*self.nq2/batch_size)
...
##########################################################################
            ...
            print ('\r i = '+str(i+1)+
                   ...
                   ', loss_reg_total =  '+str(total_loss_reg.to(device_c).data.numpy())+
                   '           .',end = ' ')
    ...
    def optim_Kinematic_reg(self, robot, q_in_reg, qtraj, q_dot, dt,q_in_boundary,q_dot_boundary,penalty,penalty_boundary, Xstable,alpha_kinematic, learning_rate = 1e-3, epoch=10000,
        ...
        for i in range(epoch):
            ...
            loss_reg = Integral_approximation/batch_size#*self.dA/2*(self.nq1*self.nq2/batch_size)
