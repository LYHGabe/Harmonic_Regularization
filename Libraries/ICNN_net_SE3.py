...
class Weightsum_ICNN(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, bias=True):
        ...
        self.icnnx = nn.Linear(4, output_dim, bias=False)    
...
class ICNN_net(nn.Module):
    ...
    def h(self, X):
        ...
        theta_periodic= torch.cat((theta_periodic10,theta_periodic11,theta_periodic12,
               theta_periodic20,theta_periodic21,theta_periodic22),dim=1)
    ...
    def h_2d(self, X):
        ...
        #theta_periodic12 =  (torch.sin(theta_norm)/theta_norm*theta[:,2]).view(-1,1)
        #theta_periodic1 = torch.cat((theta_periodic10,theta_periodic11,theta_periodic12),dim=1)
        theta_periodic20 = ((1-torch.cos(theta_norm))/(theta_norm**2)*(theta[:,0]**2)).view(-1,1)
        theta_periodic21 = ((1-torch.cos(theta_norm))/(theta_norm**2)*(theta[:,1]**2)).view(-1,1)
        #theta_periodic22 = ((1-torch.cos(theta_norm))/theta_norm*(theta[:,2]**2)).view(-1,1)
        #theta_periodic2 = torch.cat((theta_periodic20,theta_periodic21,theta_periodic22),dim=1)
    ...
    def V_forward(self, X, Xstable):
        ...
        V = Vsum#self.Smooth_ReLU(V_zf) + Vsum
        return V
    ...
    def f_forward(self, X, Xstable):
        ...
        X_se3 = self.h_2d(X)
        Xstable_se3 = self.h_2d(Xstable.view(-1,2)).view(4)
        V = self.V_forward(X_se3, Xstable_se3)
        f_hat = self.fhat_forward(X_se3)
