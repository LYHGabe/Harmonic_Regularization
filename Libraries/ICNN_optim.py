...
class ICNN_optim:
    ...
    def __init__(self,V_hidden_sizes, fhat_hidden_sizes,tol,alpha,Xstable):
    ...
    def get_boundary(self,nq1_b,nq2_b,qmin,qmax):
        ...
        q_in_boundary = torch.zeros((nq1_b*2+nq2_b*2),2,device=device, dtype=dtype)
        ...
        boundary_speed = 1.0
        q_dot_boundary  = torch.zeros((nq1_b*2+nq2_b*2),2,device=device, dtype=dtype)
    ...
    def optim_Euc_reg(self,q_in_reg,qtraj ,q_dot,dt,q_in_boundary,q_dot_boundary, learning_rate = 1e-2,epoch=10000,
        ...
        for t in range(epoch):
            for i, data in enumerate(train_loader):
                ...
                penalty = 1000
                eps = 0.01
                loss = loss_reg+penalty*(F.relu(loss_task-eps)**2)+penalty_boundary*((loss_boundary)**2)
                ...
                if i%10==0:
                    total_grid = q_in_reg.to(device)
                    total_grid.requires_grad=True
                    current_output = self.model.f_forward(total_grid,self.Xstable).to(device)
    ...
    def optim_Kinematic_reg(self, robot, q_in_reg, qtraj, q_dot, dt,q_in_boundary,q_dot_boundary, Xstable, learning_rate = 1e-3, epoch=10000,
                      batch_size = 400):
    ...
    def optim_Kinetic_reg(self, robot, q_in_reg, qtraj, q_dot,dt,q_in_boundary,q_dot_boundary, Xstable, learning_rate = 1e-3, epoch=10000,
                      batch_size = 400):
